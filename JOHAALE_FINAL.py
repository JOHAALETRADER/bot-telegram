...
MENSAJE_YA_TENGO_CUENTA = """Para tener acceso a mi comunidad VIP y todas las herramientas debes realizar tu registro con mi enlace.
        ...
        await bot.send_message(chat_id, "¿Qué debes hacer?")
...
await bot.send_message(chat_id, "Para tener acceso a mi comunidad VIP y todas las herramientas debes realizar tu registro con mi enlace.")

        await bot.send_message(chat_id, "¿Qué debes hacer?")
1. Copia y pega el enlace de registro en barra de búsqueda en modo incógnito o activa una VPN para cambiar la IP.
2. Usa un correo que no hayas usado antes en Binomo.
3. Regístrate manualmente.

🔁 Si tu cuenta actual tiene fondos y puedes retirarlos, hazlo y deposítalo en tu nueva cuenta.
❌ Si no tiene fondos, elimínala después. Desde tu perfil, al pie de la página, opción BLOQUEAR CUENTA.

✅ No olvides enviarme tu ID de Binomo para validar el registro.")elif q.data == "beneficios_vip":
        await bot.send_message(chat_id, "🎁 Beneficios que recibirás:
...
Recuerda que la cantidad de beneficios varía según tu inversión personal.")elif q.data == "status_inversion":
        with open("IMG_20250626_172306_849.jpg", "rb") as img1, open("IMG_20250626_172303_416.jpg", "rb") as img2:
            media = [InputMediaPhoto(img1), InputMediaPhoto(img2)]
            await bot.send_media_group(chat_id=chat_id, media=media)elif q.data == "status_inversion":
        with open("IMG_20250626_172306_849.jpg", "rb") as img1, open("IMG_20250626_172303_416.jpg", "rb") as img2:
            media = [InputMediaPhoto(img1), InputMediaPhoto(img2)]
            await bot.send_media_group(chat_id=chat_id, media=media)async def guardar_mensaje(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
