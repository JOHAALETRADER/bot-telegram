...
engine = create_engine(DATABASE_URL, echo=False)
    try:
        ...
        print("✅ Tabla usuarios eliminada para recreación")
    except Exception as e:
        print("⚠️ Error al eliminar la tabla:", e)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    mensaje_bienvenida = MENSAJE_BIENVENIDA.format(first_name=first_name)
    context.bot.send_photo(chat_id=chat_id, photo=open('bienvenida_v20_johanna.jpg', 'rb'), caption=mensaje_bienvenida, parse_mode='HTML')
    ...
    context.bot.send_message(chat_id=chat_id, text="Selecciona una opción para continuar:", reply_markup=InlineKeyboardMarkup(kb))

    session = Session()
    nuevo_usuario = Usuario(telegram_id=chat_id, username=username, nombre=first_name)
    session.merge(nuevo_usuario)
    session.commit()
    session.close()
