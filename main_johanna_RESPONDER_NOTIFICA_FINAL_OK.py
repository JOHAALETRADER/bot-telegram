...
# 💬 Comando /responder para enviar mensajes manualmente a usuarios
@application.message_handler(filters.COMMAND)
async def comando_responder(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.text.startswith("/responder"):
        partes = update.message.text.split(" ", 2)
        if len(partes) < 3:
            await update.message.reply_text("❗ Usa el formato: /responder ID mensaje")
            return
        ...
        try:
            await context.bot.send_message(
                chat_id=user_id,
                text=f"📩 *Mensaje del soporte:*\n\n{mensaje}",
                parse_mode=ParseMode.MARKDOWN
            )
            await update.message.reply_text("✅ Mensaje enviado correctamente.")
        except Exception as e:
            await update.message.reply_text(f"⚠️ Error al enviar el mensaje: {e}")

# 📬 Notificar a Johanna cuando alguien escribe al bot
...
async def notificar_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(
            chat_id=admin_id,
            text=f"📥 Nuevo mensaje de {user.full_name} (@{user.username} | ID: {user.id}):\n\n{mensaje}"
        )
    except Exception as e:
