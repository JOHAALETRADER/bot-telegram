...
# === Comando para responder a un usuario desde el admin ===
async def responder(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(chat_id=int(user_id), text=mensaje)
        await update.message.reply_text("✅ Mensaje enviado.")
    except Exception as e:
        await update.message.reply_text(f"❌ Error al enviar: {e}")

# === Notificación automática al admin cuando un usuario escribe ===
@application.message_handler(func=lambda message: True)
async def notificar_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message and update.message.text:
        user = update.effective_user
        text = update.message.text
        mensaje = f"📩 Nuevo mensaje de @{user.username or 'Usuario'} (ID: {user.id})\n✉️ {text}"
        await context.bot.send_message(chat_id=5924691120, text=mensaje)
