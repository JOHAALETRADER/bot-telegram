...
def _set_pending_ai(chat_id: int, text_value: str, message_id: int, answered_topics=None):
    ...
    with Session() as session:
        ...
        if u.ai_pending_text and u.ai_pending_due_at and u.ai_pending_due_at >= datetime.utcnow():
...
async def live_broadcast_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    recipients = _recent_live_recipients()
    try:
        await query.edit_message_caption(caption=f"⏳ Enviando LIVE a {len(recipients)} usuarios...")
    except Exception:
        try:
            await query.edit_message_text(f"⏳ Enviando LIVE a {len(recipients)} usuarios...")
        except Exception:
            pass
...
async def marketing_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "marketing_cancel":
        ...
        try:
            ...
            await context.bot.send_message(chat_id=ADMIN_ID, text="❌ Marketing cancelado.")
        return
    ...
    recipients = _recent_marketing_recipients()
    try:
        await query.edit_message_caption(caption=f"⏳ Enviando marketing a {len(recipients)} usuarios PRE/POST...")
    except Exception:
        try:
            await query.edit_message_text(f"⏳ Enviando marketing a {len(recipients)} usuarios PRE/POST...")
        except Exception:
            pass
