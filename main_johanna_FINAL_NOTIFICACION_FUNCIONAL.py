    ...
    if str(update.effective_user.id) != ADMIN_ID:
        try:
            await context.bot.send_message(
                chat_id=ADMIN_ID,
                text=f"Nuevo mensaje de @{update.effective_user.username or update.effective_user.first_name}:

{update.message.text}",
                reply_markup=None,
            )
        except Exception as e:
            print("Error al reenviar mensaje al admin:", e)

async def reply_to_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        if update.message.reply_to_message:
            original_text = update.message.reply_to_message.text
            if "Nuevo mensaje de" in original_text:
                lines = original_text.splitlines()
                user_line = lines[0]
                username = user_line.split("de @")[1].split(":")[0]
                for dialog in context.bot_data.get("dialogs", []):
                    if dialog.get("username") == username:
                        user_id = dialog["user_id"]
                        await context.bot.send_message(
                            chat_id=user_id,
                            text=update.message.text
                        )
                        await context.bot.send_message(chat_id=ADMIN_ID, text="✅ Mensaje enviado.")
                        return
                await context.bot.send_message(chat_id=ADMIN_ID, text="❌ No se pudo enviar el mensaje.")
    except Exception as e:
        print("Error en reply_to_user:", e)

if __name__ == "__main__":
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, forward_message_to_admin))
    application.add_handler(MessageHandler(filters.TEXT & filters.REPLY, reply_to_user))
