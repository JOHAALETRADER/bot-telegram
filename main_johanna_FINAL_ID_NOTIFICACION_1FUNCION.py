...
async def forward_message_to_admin(update: Update, context: CallbackContext):
    user = update.effective_user
    text = update.message.text
    await context.bot.send_message(
        chat_id=ADMIN_ID,
        text=f"📩 Nuevo mensaje de {user.first_name} (@{user.username} | ID: {user.id}):\n{text}",
        reply_markup=None
    )
...
async def reply_to_user(update: Update, context: CallbackContext):
    if update.message.reply_to_message:
        try:
            target_id_line = update.message.reply_to_message.text.split("ID: ")[-1]
            target_id = int(target_id_line.split("):")[0])
            await context.bot.send_message(chat_id=target_id, text=update.message.text)
            await context.bot.send_message(chat_id=ADMIN_ID, text="✅ Mensaje enviado con éxito.")
        except Exception as e:
            await context.bot.send_message(chat_id=ADMIN_ID, text="❌ Error al enviar el mensaje.")
