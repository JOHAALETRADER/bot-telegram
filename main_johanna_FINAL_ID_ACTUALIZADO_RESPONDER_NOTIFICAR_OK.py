...
async def handle_incoming_message(message: types.Message):
    if message.chat.type == "private" and message.from_user.id != 5924691120:
        await bot.send_message(
            chat_id=5924691120,
            text=f"📩 Nuevo mensaje de @{message.from_user.username or message.from_user.id}:

{message.text}"
        )

# ------------------- RESPONDER AL USUARIO ----------------------
@dp.message_handler(commands=['responder'])
async def responder_a_usuario(message: types.Message):
    if message.from_user.id != 5924691120:
        return
    try:
        partes = message.text.split(maxsplit=2)
        if len(partes) < 3:
            await message.reply("❗ Usa el formato: /responder <user_id> <mensaje>")
            return
