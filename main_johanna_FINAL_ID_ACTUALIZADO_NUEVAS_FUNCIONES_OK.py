...
# 🔔 Nueva función: Notificar al admin cuando alguien escribe
async def notificar_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message:
        user = update.effective_user
        admin_id = ADMIN_ID
        mensaje = f"📨 Nuevo mensaje de {user.first_name} (@{user.username if user.username else 'sin usuario'}):\n\n{update.message.text}"
        await context.bot.send_message(chat_id=admin_id, text=mensaje)
        context.user_data["from_user"] = user.id  # Guarda el ID del usuario para responder

# ✅ Nueva función: Responder al usuario original
async def responder_a_usuario(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.reply_to_message:
        texto = update.message.text
        usuario_id = context.user_data.get("from_user")
        if usuario_id:
            try:
                await context.bot.send_message(chat_id=usuario_id, text=texto)
                await update.message.reply_text("✅ Mensaje enviado con éxito.")
            except Exception as e:
                await update.message.reply_text("❌ Error al enviar el mensaje.")
        else:
            await update.message.reply_text("⚠️ No se pudo identificar al usuario para responder.")
